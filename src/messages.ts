...
const description = (desc?: string | null): string => desc ? `\n📝 ${desc}` : "";
export const MSG = {
  home: "Aeolipyle 🌀 — a slash-command counter bot for Discord",
  created: (name: string, desc?: string | null) => `✨ Created counter **${name}** starting at **0**!${description(desc)}`,
  alreadyExists: (name: string) => `⚠️ A counter named **${name}** already exists here.`,
  restorePrompt: (name: string, deletedAt: number, last: number) => `♻️ **${name}** was removed ${discordRelativeTimestamp(deletedAt)} and was at **${formatCount(last)}**. Bring it back?`,
  restored: (name: string, mode: RestoreMode, count: number) => `♻️ Restored counter **${name}** at **${formatCount(count)}** (${mode === "zero" ? "reset to zero" : "kept its old count"})`,
  ...
  removed: (name: string, count?: number) => `🗑️ Removed counter **${name}**${count === undefined ? "" : ` (it was at **${formatCount(count)}**)`}. \`/addcounter\` can bring it back.`,
  ...
  renameTargetDeleted: (name: string, deletedAt: number) => `⚠️ **${name}** belongs to a counter removed ${discordRelativeTimestamp(deletedAt)}. Restore it with \`/addcounter\` or pick another name.`,
  counterStatus: (name: string, count: number, desc?: string | null) => `🔢 **${name}** is at **${formatCount(count)}**${description(desc)}`,
...
export function renderRestorePrompt(name: string, deletedAt: number, count: number, token: string): MessageBody {
  return { content: MSG.restorePrompt(name, deletedAt, count), components: [{ type: ComponentType.ActionRow, components: [
...
export function renderCountersPage(page: CounterPage, counts: number[]): MessageBody {
  ...
  return { content: "", embeds: [{ title: "📋 Counters", description: page.items.map((item, i) => `**${item.displayName}** · ${formatCount(counts[i] ?? 0)}`).join("\n"), footer: { text: `Page ${page.page + 1}/${page.pageCount} · ${page.total} counters` } }], components: page.pageCount <= 1 ? [] : [{ type: ComponentType.ActionRow, components: [
    { type: ComponentType.Button, style: ButtonStyle.Secondary, label: "◀️ Previous", custom_id: pageId(Math.max(0, page.page - 1)), disabled: page.page === 0 },
    { type: ComponentType.Button, style: ButtonStyle.Secondary, label: "Next ▶️", custom_id: pageId(page.page + 1), disabled: page.page === page.pageCount - 1 },
  ] }] };
}
